mdes.crd4r4 <- function(power = .80, alpha = .05, two.tailed = TRUE, df = n4 - g4 - order - 2,
  ...
  sse <- (1/(rate.tp - rate.cc)) * sqrt(d * (rho4 * (1 - r24) / (p * (1 - p) * n4) +
...
power.crd4r4 <- function(score = NULL, order = 2, rhots = NULL, k1 = -6, k2 = 6, dists = "normal",
...
cosa.crd4r4 <- function(score = NULL, order = 2, rhots = NULL, k1 = -6, k2 = 6, dists = "normal",
  ...
  fun <- "cosa.crd4r4"
  lb <- c(1, 1, 1, g4 + order + 3)

  .df <- quote(n4 - g4 - order - 2)
